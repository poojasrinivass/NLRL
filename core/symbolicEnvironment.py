...
LPUSH = Predicate("lpush", 0)       #left push the pole, action with force = -10
RPUSH = Predicate("rpush", 0)       #right push the pole, action with force = 10
CARPOS = Predicate("carpos",1)      #car position while moving with a velocity
CARVEL = Predicate("carvel", 1)     #car velocity is varying
POLEANG = Predicate("poleang", 1)   #pole angle from the upright centre
POLEVEL = Predicate("polevel", 1)   #pole velocity at tip of pole

class cartPole(SymbolicEnvironment):
    def __init__(self):
        actions = [LPUSH, RPUSH]
        self.gravity = 9.8
        self.masscart = 1.0
        ...
        self.polemass_length = 0.5
        self.force_mag = 10
        self.theta_threshold = 15*math.pi/180
        self.threshold = 2.4
        self.x_dot = 0.2
        self.theta_dot = 03
        background = []
        self.unseen_background = []
        self.unseen_background.extend([Atom(CARPOS, [str(x), "0"]) for x in range(1, threshold)])
        # background.extend([Atom(SUCC, [str(i), str(i+1)]) for i in range(width - 1)])

        super(cartPole, self).__init__(background, initial_state, actions)
        

        self.language = LanguageFrame(actions, extensional=[CARPOS, CARVEL, POLEVEL, POLEANG])
        high = np.array[self.threshold*2, np.finfo(np.float32).max, self.theta_threshold*2, np.finfo(np.float32).max,]

    @property
    def all_states(self):
        return[str(i) for i in range(self.high)]

    def action_vec2symbol(self, action_vec):
        return self.actions[np.argmax(action_vec)]
